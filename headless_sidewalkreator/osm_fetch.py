...
def get_osm_data(bbox: Tuple[float, float, float, float], tags: Optional[Dict[str, object]] = None, timeout: int = 60, max_retries: int = 2) -> gpd.GeoDataFrame:
    ...
    try:
        ...
        logger.info(f"Fetching OSM data for bbox {bbox} with timeout {ox.settings.requests_timeout}s")
        ...
        while attempt <= max_retries:
            try:
                ...
                last_exc = exc
                logger.warning("OSM data fetch failed on attempt %d: %s", attempt + 1, exc)
                attempt += 1
